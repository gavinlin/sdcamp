...
分布式顾名思议就是代码仓库是可以分布在各处的，那么你就可以做很多以前必须要配置管理员参与的事情，如分支。当然它也带来一定的复杂性，早期可能还不太适应。有兴趣的朋友可以看看我在图灵写的文章[企业版本控制的改革：从ClearCase到Git--我的布道之旅](http://www.ituring.com.cn/article/721)。
...
### 建立本地 Git 仓库 ###
既然是分布式，就可以直接干活了。创建一个干净目录`helloworld`
...
### Git标记（Tag）###
一般在发布前，我们需要打一个标记（Tag），表明这是一个重要的点，以后可以很方便地把当前的状态恢复，省得记录固定的某个commit了。
...
代码审阅是一个不错的敏捷开发实践，但实施却往往让人非常头疼。大企业中通常是制定出一大堆相关的规范和流程来指导代码审阅。谷歌的 Android 系统是现在非常热门的开源项目，它的代码审阅（包括贡献者的代码）使用的是Gerrit，非常棒的东西，在自己的企业中架设起来也非常方便。

Insert 18333fig0302.png
图 3-2. Gerrit代码审阅系统

Gerrit是一个基于 Web 的代码评审和项目管理的工具，面向基于 Git 版本控制系统的项目，所以如果你没用git，就没法用Gerrit了，接下来看看在Gerrit中是怎么实施代码评审的。
